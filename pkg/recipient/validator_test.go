...
// TestValidator_AcceptRecipient tests accepting a valid recipient
func TestValidator_AcceptRecipient(t *testing.T) {
	...
	if resp.Message != "Recipient accepted" {
		t.Errorf("Unexpected message: %s", resp.Message)
	}

	t.Log("✓ Accepting valid recipient works")
}

// TestValidator_RejectRecipient_NotFound tests rejecting non-existent recipient
func TestValidator_RejectRecipient_NotFound(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	ctx := context.Background()
	resp, err := validator.Validate(ctx, "192.168.1.1", "sender@example.com", "nonexistent@example.com")
	if err != nil {
	...
	if resp.Message != "User unknown" {
		t.Errorf("Expected 'User unknown', got %s", resp.Message)
	}

	t.Log("✓ Rejecting non-existent recipient works")
}

// TestValidator_RejectRecipient_Forbidden tests blocking by recipient
func TestValidator_RejectRecipient_Forbidden(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	if resp.Message != "Sender blocked by recipient" {
		t.Errorf("Unexpected message: %s", resp.Message)
	}

	t.Log("✓ Blocking by recipient works")
}
...
func TestValidator_RateLimit(t *testing.T) {
	...
	ctx := context.Background()
	_, err = validator.Validate(ctx, "192.168.1.1", "sender@example.com", "recipient@example.com")
	if err == nil {
...
func TestValidator_URLInterpolation(t *testing.T) {
	...
	ctx := context.Background()
	_, err = validator.ValidateWithContext(ctx, "192.168.1.1", "mail.example.com", "helo.example.com", "sender@example.com", "recipient@example.com")
	if err != nil {
		t.Fatalf("Validation failed: %v", err)
	}
	...
	expectedParams := map[string]string{
		...
		"email": "recipient%40example.com", // URL-encoded @
	}
...
func TestValidator_Caching(t *testing.T) {
	...
	resp1, err := validator.Validate(ctx, "192.168.1.1", "sender@example.com", "recipient@example.com")
	if err != nil {
		t.Fatalf("First validation failed: %v", err)
	}

	// Second request - should use cache
	resp2, err := validator.Validate(ctx, "192.168.1.1", "sender@example.com", "recipient@example.com")
	if err != nil {
...
// TestValidator_NoCacheForRejected tests that rejected recipients are not cached
func TestValidator_NoCacheForRejected(t *testing.T) {
	...
	// Make multiple requests for rejected recipient
	for i := 0; i < 3; i++ {
		_, err = validator.Validate(ctx, "192.168.1.1", "sender@example.com", "nonexistent@example.com")
		if err != nil {
